...
- **Don't forget to bump `plugin.json` `version` in any PR touching `plugins/facets/`.** `/plugin marketplace update` keys cache invalidation off this field — leave it the same and every existing install keeps serving the old description, old agent roster, old hook script. This bit us once: 2.3.0 sat unchanged for two days while the description and the agents/ layout were rewritten in place; users kept seeing the original 11-persona text from the May 19 install. See the Versioning section for semver rules (patch/minor/major). **Enforced:** `test/plugin.bats` now fails when a branch changes `plugins/facets/` without bumping `plugin.json` past `origin/main`'s version (it skips on `main`, when `origin/main` is unreachable, and for PRs that don't touch `plugins/facets/`). **Stacked PRs:** the version line collides by design — `main` *is* the released marketplace, so every PR must bump it; resolve the conflict to **one above whatever `main` currently is**, and the guard confirms it. (This is why there's no `@changesets`-style deferred-bump flow — there's no separate release step to defer to.)
