...
Each phase below follows a **test-driven block loop**. Do not advance to the next phase until every checkbox in the current phase's *Block validation* sub-section is green. `/local:tib-ship` automates this loop; humans running it manually should follow the same order.
