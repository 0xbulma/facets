  ...
  "comments": [
    {
      ...
      "line": <snapped_line>,
      "side": "RIGHT",
      "body": "**[SEVERITY]** (confidence: <NN>%) [<agents>] <description>\n\nSuggestion: <how to fix>"
    }
  ]
}
```
...
**Confidence token `(confidence: <NN>%)`.** Render each finding's `confidence` (from the engine's Step 6) as `(confidence: <NN>%)` immediately after `**[SEVERITY]**` and before the `[<agents>]` token — e.g. `**[HIGH]** (confidence: 72%) [web3]`. If `confidence` is absent (the agent stated none, or a legacy cached-ledger reuse), omit the token rather than emitting `(confidence: unstated)`. Confidence is triage metadata for the reader — it is **never** used to suppress a comment; every kept finding is posted regardless of its confidence.

**Attribution token `[<agents>]`.** Render each finding's `agents` array (from the engine's Step 6) as a comma-separated bracket token after the confidence token — e.g. `**[HIGH]** (confidence: 72%) [web3]`, or `**[HIGH]** (confidence: 72%) [web3, correctness]` when several personas raised it. If `agents` is empty or absent (e.g. a cached-ledger reuse of a legacy entry with no attribution), omit the token rather than emitting `[]`. This is orthogonal to the `[NEW]` net_new tag (Step 5b), which stays where it already renders.
