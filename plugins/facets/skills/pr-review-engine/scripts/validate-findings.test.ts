...
function run(input: RunInput): Out {
	return validateFindingsFromText({
		...
		changedLinesText: input.changedLinesText ?? JSON.stringify(input.changedLines ?? {}),
...
describe("schema validation", () => {
	...
	it("fails schema when missing WHAT:", () => {
		const out = ok(
			run({
				findings: [{ severity: "high", file: "src/X.ts", line: 10, description: "FIX: change." }],
	...
	it("fails schema when missing FIX:", () => {
		const out = ok(
			run({
				findings: [
					{ severity: "high", file: "src/X.ts", line: 10, description: "WHAT: thing happened." },
				],
				changedLines: { "src/X.ts": [10] },
			}),
		);
	...
	it("fails schema for line: 0 on a real file (only valid on runtime)", () => {
		const out = ok(
			run({
				findings: [
					{ severity: "high", file: "src/X.ts", line: 0, description: "WHAT: x. FIX: y." },
				],
				changedLines: { "src/X.ts": [10] },
			}),
		);
...
describe("scope filter", () => {
	it("drops a file out of scope", () => {
		const out = ok(
			run({
				findings: [
					{ severity: "medium", file: "other/Y.ts", line: 5, description: "WHAT: x. FIX: y." },
				],
				changedLines: { "src/X.ts": [10] },
			}),
		);
	...
	it("keeps a finding exactly at the +15 boundary (inclusive)", () => {
		const out = ok(
			run({
				findings: [
					{ severity: "low", file: "src/X.ts", line: 25, description: "WHAT: x. FIX: y." },
				],
				changedLines: { "src/X.ts": [10] },
			}),
		);
	...
	it("drops a finding at the +16 boundary, tagged with the distance", () => {
		const out = ok(
			run({
				findings: [
					{ severity: "low", file: "src/X.ts", line: 26, description: "WHAT: x. FIX: y." },
				],
				changedLines: { "src/X.ts": [10] },
			}),
		);
	...
	it("keeps any finding on a pure-rename file (empty changed set)", () => {
		const out = ok(
			run({
				findings: [
					{ severity: "medium", file: "src/X.ts", line: 999, description: "WHAT: x. FIX: y." },
				],
	...
	it("schema-only mode skips the scope filter", () => {
		const out = ok(
			run({
				findings: [
					{ severity: "low", file: "other/Y.ts", line: 1, description: "WHAT: x. FIX: y." },
				],
				changedLines: { "src/X.ts": [10] },
				schemaOnly: true,
			}),
	...
	it("honors a custom line tolerance", () => {
		...
		expect(
			ok(run({ findings: [finding], changedLines: { "src/X.ts": [10] }, lineTolerance: 20 })).kept,
		).toHaveLength(1);
		expect(ok(run({ findings: [finding], changedLines: { "src/X.ts": [10] } })).kept).toEqual([]);
	});
});
...
describe("path normalization", () => {
	...
	it("strips a leading ./", () => {
		const out = ok(
			run({
				findings: [
					{ severity: "medium", file: "./src/X.ts", line: 10, description: "WHAT: x. FIX: y." },
				],
				changedLines: { "src/X.ts": [10] },
			}),
		);
	...
	it("drops an absolute path outside the repo root", () => {
		...
		const out = ok(
			run({
				findings: [{ severity: "medium", file: stray, line: 10, description: "WHAT: x. FIX: y." }],
				changedLines: { "src/X.ts": [10] },
				repoRoot: root,
			}),
		);
...
describe("markdown documentation-example filter", () => {
	const reader = (content: string) => (path: string) => (path.endsWith("docs.md") ? content : null);
...
describe("runtime-validation sentinel", () => {
	...
	it("still requires WHAT:/FIX: clauses", () => {
		const out = ok(
			run({
				findings: [
					{ severity: "high", file: "runtime", line: 0, description: "the page looked broken" },
				],
				changedLines: { "src/X.ts": [10] },
			}),
		);
	...
	it("fails schema on a negative line", () => {
		const out = ok(
			run({
				findings: [
					{ severity: "high", file: "runtime", line: -1, description: "WHAT: x. FIX: y." },
				],
				changedLines: { "src/X.ts": [10] },
			}),
		);
	...
	it("keeps a positive line and still bypasses scope filters", () => {
		const out = ok(
			run({
				findings: [{ severity: "high", file: "runtime", line: 7, description: "WHAT: x. FIX: y." }],
				changedLines: { "src/X.ts": [10] },
			}),
		);
		expect(out.kept).toHaveLength(1);
...
describe("tolerant agent-output parser", () => {
	...
	it("recovers a prose-wrapped empty array", () => {
		const out = ok(
			run({ findingsText: "All checks pass, nothing to report.\n\n[]\n", changedLines: {} }),
		);
		expect(out.kept).toEqual([]);
	...
	it("recovers an object-wrapped array (sole list value)", () => {
		const out = ok(run({ findingsText: `{"findings": [${FINDING}]}`, changedLines: CL_X }));
		expect(out.kept).toHaveLength(1);
	});
	...
	it("recovers a prose-wrapped sole-list dict", () => {
		const out = ok(
			run({ findingsText: `Here are my results.\n{"findings": [${FINDING}]}`, changedLines: CL_X }),
		);
		expect(out.kept).toHaveLength(1);
	});
	...
	it("rejects a truncated array", () => {
		const out = run({
			findingsText: 'Truncated: [{"severity": "high", "file": "x.ts", "line": 1]',
			changedLines: {},
		});
	...
	it("never unwraps a failure-named sole key", () => {
		for (const payload of [
			...
			expect(hasError(run({ findingsText: payload, changedLines: CL_X }))).toBe(true);
		}
	});

	it("never unwraps a prose-wrapped failure-named sole key", () => {
		expect(
			hasError(run({ findingsText: 'Hit the limit.\n{"partial_findings": []}', changedLines: {} })),
		).toBe(true);
	});

	it("rejects an ambiguous multi-list dict", () => {
		expect(
			hasError(run({ findingsText: '{"findings": [], "skipped": []}', changedLines: {} })),
		).toBe(true);
	});
	...
	it("returns a structured error for invalid changed-lines JSON", () => {
		const out = run({ findingsText: "[]", changedLinesText: "not json at all" });
		expect(hasError(out)).toBe(true);
		if ("error" in out) expect(out.error).toContain("invalid changed-lines JSON");
	});
...
describe("CLI shell", () => {
	...
	it("returns a structured error for a missing findings file", () => {
		const res = cli(["--findings", "/no/such/file.json", "--changed-lines", clFile()], "");
		expect(res.status).toBe(0);
