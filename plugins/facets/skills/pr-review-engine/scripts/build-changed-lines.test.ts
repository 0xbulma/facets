...
describe("mergeChangedLines", () => {
	...
	it("adds renamed paths as empty-array keys", () => {
		expect(mergeChangedLines({ committed: {}, uncommitted: {}, renamed: ["renamed.txt"] })).toEqual(
			{
				"renamed.txt": [],
			},
		);
	...
	it("does not overwrite an existing key's lines with a rename entry", () => {
		expect(
			mergeChangedLines({ committed: { "r.txt": [10] }, uncommitted: {}, renamed: ["r.txt"] }),
		).toEqual({ "r.txt": [10] });
	});
});
...
describe("build-changed-lines CLI (real git)", () => {
	...
	function build(dir: string, args: readonly string[]): Record<string, number[]> {
		const res = spawnSync("node", [SCRIPT, ...args], { cwd: dir, encoding: "utf8" });
		if (res.status !== 0)
			throw new Error(`build-changed-lines exited ${res.status}: ${res.stderr}`);
	...
	it("maps a simple addition hunk to the expected line numbers", () => {
		...
		expect(build(dir, ["--base", base, "--head", "HEAD"])["foo.txt"]).toEqual([3, 4]);
	});
	...
	it("anchors a deletion-only hunk at line 1", () => {
		...
		expect(build(dir, ["--base", base, "--head", "HEAD"])["foo.txt"]).toEqual([1]);
	});
	...
	it("parses a single-line modify (omitted count) as one line", () => {
		...
		expect(build(dir, ["--base", base, "--head", "HEAD"])["foo.txt"]).toEqual([2]);
	});
	...
	it("--include-uncommitted unions a staged change", () => {
		...
		const local = build(dir, ["--base", base, "--head", "HEAD", "--include-uncommitted"]);
	...
	it("unions committed + uncommitted renames", () => {
		...
		const out = build(dir, ["--base", base, "--head", "HEAD", "--include-uncommitted"]);
