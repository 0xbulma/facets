...
export function parseDiff(diffText: string): Record<string, number[]> {
	...
	for (const line of diffText.split("\n")) {
		...
		if (line.startsWith("@@ ") && file !== "") {
			...
			const newCount = countText === undefined ? 1 : Number.parseInt(countText, 10) || 0;
...
function getRenames(range: string): string[] {
	const buf = execFileSync("git", ["diff", "-z", "--name-only", "--diff-filter=R", range], {
...
function main(): number {
	...
	const uncommitted = parsed.includeUncommitted
		...
		: {};
	const renamed = [...getRenames(range), ...(parsed.includeUncommitted ? getRenames("HEAD") : [])];
