 ...
 */
export function runHash(parts: { mergeBase: string; headSha: string; diff: string }): string {
...
type Mode = { kind: "to-https"; url: string } | { kind: "run-hash"; base: string };

export function parseArgs(argv: readonly string[]): Mode {
	const args = [...argv];
	if (args[0] === "--to-https") {
		const url = args[1];
		if (url === undefined) throw new UsageError("--to-https requires a remote URL");
	...
	if (args[0] === "--run-hash") {
		...
		if (base === undefined) throw new UsageError("--run-hash requires --base <merge-base>");
		return { kind: "run-hash", base };
	}
	throw new UsageError("expected --to-https <url> or --run-hash --base <merge-base>");
}
