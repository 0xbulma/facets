...
export function schemaOk(finding: unknown): finding is ValidatedFinding {
	...
	if (typeof severity !== "string" || !VALID_SEVERITIES.has(severity)) return false;
	...
	if (file === "runtime") {
		if (typeof line !== "number" || !Number.isInteger(line) || line < 0) return false;
 ...
 */
export function validateFindingsFromText(opts: ValidateOptions): ValidationResult | ErrorResult {
	...
	try {
		...
		return { error: `invalid changed-lines JSON: ${e instanceof Error ? e.message : String(e)}` };
	}
	...
	const counts: Counts = { out_of_scope: 0, pre_existing: 0, doc_example: 0, schema: 0 };
...
export function parseArgs(argv: readonly string[]): CliArgs {
		...
		switch (arg) {
			...
			case "--line-tolerance": {
				...
				if (!Number.isInteger(parsed)) {
					throw new UsageError(`--line-tolerance must be an integer, got "${value}"`);
				}
	...
	if (changedLines === undefined) throw new UsageError("--changed-lines is required");
