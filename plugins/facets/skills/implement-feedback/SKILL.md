...
3. **On any non-success sentinel** (`GOAL_INCOMPLETE`, `GOAL_ABORTED`, `GOAL_STUCK`, `GOAL_MAXED`, `GOAL_RUNTIME_RED`) → **do NOT open the PR.** Surface the sentinel and the residual findings, and stop for the user. The branch is left at its last green commit (the loop's own clean-up guarantees this) — **except** on the commit-failure `GOAL_ABORTED`, which deliberately leaves that iteration's fixes staged and uncommitted; tell the user to commit them by hand (not stash) before re-running. The user can re-run with a higher `--max-iters`, fix the sticking point, or open the PR by hand.
