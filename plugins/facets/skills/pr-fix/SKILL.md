...
```
/facets:pr-fix <pr-number>
/facets:pr-fix <pr-number> --watch
```
...
```
Working tree is not clean. Please commit or stash your changes before running /facets:pr-fix.
```
...
- **Pairs with `/facets:pr-review-gh`**: `/facets:pr-review-gh` posts findings (from parallel Claude agents + optional Codex), `/facets:pr-fix` applies fixes. **Do NOT run both watchers on the same PR** — each fix push re-triggers the review watcher and each new finding re-triggers this watcher: an unattended ping-pong loop. Watch with one skill at a time and run the other on demand.
