...
Then run `couple-sweep.ts --base <merge-base> --changed-lines <map>` once per review and merge its array into the findings, stamped `agents:["couple-sweep"]`. It derives file pairings from `git log` before the merge base and reports every changed file whose strongly-coupled partner is absent from the diff — one exhaustive pass instead of a reviewer rediscovering mirror drift one instance per round. It never compares the contents of two changed partners. A failure or empty result is not a finding; continue with `[]`.
...
For any “until clean” loop, discover commands first. Run the baseline test command once: if already red, show exact failures and ask whether to stop or proceed; on proceed, record them and require every later gate to introduce no new failures. Default five iterations (`--max-iters` overrides): static review → decide → fix critical/high/medium per-file atomically → gates → re-review. Never auto-fix lows.

Do not re-derive the stop conditions. After each iteration's review, pipe the state to `../../plugins/facets/skills/pr-review-engine/scripts/goal-loop.ts` on stdin — `{iteration, max_iters, failed_agents, total_agents_launched, prev_actionable_hash, findings, head_branch, base_branch}` — and obey the returned `action`: `fix` continues; `converged`, `incomplete`, `stuck`, and `maxed` each stop after printing the returned `sentinel`. Carry `actionable_hash` forward as the next call's `prev_actionable_hash`. `maxed` stops before that round's fixes, so the residual findings match the tree. Evaluate the same five conditions by hand only when the helper cannot run, and say so.
