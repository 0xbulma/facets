...
Use only dummy/local accounts; never request, print, store, or inject a real private key. Treat RPC URLs as secrets: redact them from output. `--impersonate` is read-only and cannot sign/send.
