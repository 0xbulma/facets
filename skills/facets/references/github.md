...
Apply high-confidence fixes; apply medium-confidence fixes but flag them for double-checking; skip low-confidence or mutually contradictory requests. Focused then project validation must pass before any fix commit or push. Stage exact fix files and create a single fix commit only when the staged diff is nonempty, then push.
