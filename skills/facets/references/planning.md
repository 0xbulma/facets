...
Then run the cumulative review/fix loop in `review.md`, selecting its commit-authorized variant; complete convergence requires `FAILED_AGENTS == 0`. Every green review-fix commit includes a `TIB: <TIB-ID>` trailer. Retain green phase/review commits and discard only the current uncommitted iteration on failure. The same finding hash twice stops stuck and asks for direction. At the iteration ceiling, print residual findings and ask whether to extend, accept-and-continue with the branch marked not review-clean, or stop. Never auto-fix lows. Run runtime once after complete static convergence for changed UI unless disabled. `tib-ship` never uses the `pr-review-local --goal` GitHub/push exception. Set each completed TIP to `In Progress` and report branch, TIPs, phases, gates, iterations, residual lows, runtime, and commits. End with manual push and `$facets pr-create` suggestions; do neither.
